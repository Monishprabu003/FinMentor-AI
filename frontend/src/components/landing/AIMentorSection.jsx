...
export const AIMentorSection = () => {
    ...
    return (<section id="ai-mentor" className="py-24 bg-white">
      <div className="max-w-7xl mx-auto px-6">
        ...
        ].map((p) => (<li key={p} className="flex items-center gap-3 text-[14px] text-slate-600 font-medium">
          ...
          <motion.div initial={{ opacity: 0, x: 24 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true }}>
            <div className="bg-white rounded-3xl border border-gray-100 shadow-xl overflow-hidden">
              ...
              <div className="p-5 space-y-4">
                {CONVERSATION.map((msg, i) => (<motion.div key={i} initial={{ opacity: 0, y: 10 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: i * 0.2 + 0.2 }} className={`flex ${msg.role === 'user' ? 'justify-end' : 'gap-2.5 items-start'}`}>
                    {msg.role === 'ai' && (<div className="w-7 h-7 rounded-full bg-blue-50 border border-blue-100 flex items-center justify-center shrink-0 mt-0.5">
                        <Bot className="w-3.5 h-3.5 text-blue-600"/>
                      </div>)}
                    <div className={`text-[13px] leading-relaxed px-4 py-3 rounded-2xl max-w-[85%] ${msg.role === 'user'
              ...
              <div className="px-5 pb-3 flex flex-wrap gap-2">
                {PROMPTS.map((p) => (<button key={p} onClick={() => setInput(p)} className="text-[11px] font-semibold text-blue-600 bg-blue-50 hover:bg-blue-100 border border-blue-100 px-3 py-1.5 rounded-full transition-colors">
                    {p}
                  </button>))}
              </div>
              ...
              <div className="border-t border-gray-100 p-4 flex gap-3">
                <input type="text" value={input} onChange={(e) => setInput(e.target.value)} placeholder="Ask your financial question..." className="flex-1 text-[13px] text-slate-700 placeholder:text-slate-400 bg-slate-50 border border-gray-100 rounded-2xl px-4 py-2.5 focus:outline-none focus:border-blue-300 focus:ring-2 focus:ring-blue-50 transition-all"/>
