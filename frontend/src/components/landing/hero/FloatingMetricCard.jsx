...
import { ShieldCheck, Sparkles, CheckCircle2, PieChart, ArrowUpRight } from 'lucide-react';
/* ── Tiny Animated SVG Line Chart ── */
const MiniLineChart = () => (<div className="w-20 h-7 relative">
    <svg className="w-full h-full overflow-visible" viewBox="0 0 80 28" fill="none">
      ...
      <path d="M 0 22 Q 15 18 25 12 T 50 14 T 80 4 L 80 28 L 0 28 Z" fill="url(#chartFill)"/>
      <motion.path d="M 0 22 Q 15 18 25 12 T 50 14 T 80 4" stroke="#2563eb" strokeWidth="2.5" strokeLinecap="round" fill="none" initial={{ pathLength: 0 }} animate={{ pathLength: 1 }} transition={{ duration: 2.2, repeat: Infinity, repeatDelay: 1.5, ease: 'easeInOut' }}/>
    </svg>
  </div>);
export const FloatingMetricCard = ({ type, className = '', style = {}, floatDelay = 0, floatDuration = 4.5, }) => {
    return (<motion.div style={style} animate={{ y: [0, -7, 0] }} transition={{
            duration: floatDuration,
            delay: floatDelay,
            repeat: Infinity,
            ease: 'easeInOut',
        }} whileHover={{ scale: 1.05, y: -4, transition: { duration: 0.2 } }} className={`absolute bg-white/95 backdrop-blur-xl border border-slate-100/90 rounded-2xl shadow-xl shadow-slate-200/40 p-3.5 sm:p-4 select-none cursor-pointer z-20 hover:border-blue-200 transition-colors ${className}`}>
      {/* 1. FINANCIAL SCORE CARD */}
      {type === 'score' && (<div className="flex items-center gap-3 min-w-[170px]">
      ...
      {type === 'budget' && (<div className="flex items-center gap-3 min-w-[175px]">
      ...
      {type === 'savings' && (<div className="flex items-center justify-between gap-3 min-w-[175px]">
          <div>
      ...
      {type === 'investment' && (<div className="flex items-center gap-3 min-w-[190px]">
          <div>
      ...
      {type === 'expenses' && (<div className="flex items-center gap-3 min-w-[185px]">
      ...
      {type === 'insights' && (<div className="flex items-center gap-3 min-w-[180px]">
