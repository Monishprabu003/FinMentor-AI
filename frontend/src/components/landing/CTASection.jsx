...
export const CTASection = ({ onGetStarted, onLearnMore }) => (<section className="py-24 bg-blue-600 relative overflow-hidden">
    {/* Subtle grid overlay */}
    ...
    <div className="relative z-10 max-w-4xl mx-auto px-6 text-center">
      <motion.div initial={{ opacity: 0, y: 24 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} className="space-y-7">
        {/* Icon */}
        ...
        <div className="flex flex-col sm:flex-row items-center justify-center gap-4 pt-2">
          <button onClick={onGetStarted} className="flex items-center gap-2 px-8 py-3.5 text-[14px] font-black text-blue-700 bg-white hover:bg-blue-50 rounded-2xl transition-colors shadow-md">
            Create Free Account <ArrowRight className="w-4 h-4"/>
          </button>
          <button onClick={onLearnMore} className="px-8 py-3.5 text-[14px] font-bold text-white border-2 border-blue-400 hover:border-white hover:bg-blue-700 rounded-2xl transition-all">
            Explore Features
          </button>
        </div>
