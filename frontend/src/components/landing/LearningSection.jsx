...
export const LearningSection = () => (<section id="learn" className="py-24 bg-slate-50 border-y border-gray-100">
    <div className="max-w-7xl mx-auto px-6">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
        {cats.map((c, i) => (<motion.div key={c.label} initial={{ opacity: 0, scale: 0.94 }} whileInView={{ opacity: 1, scale: 1 }} viewport={{ once: true }} transition={{ delay: i * 0.05, duration: 0.35 }} whileHover={{ y: -4 }} className={`flex flex-col items-center gap-3.5 p-6 rounded-2xl border ${c.bdr} ${c.bg} ${c.hover} transition-all duration-200 cursor-pointer`}>
