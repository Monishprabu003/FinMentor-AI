...
const ProgressRing = ({ progress, size = 36 }) => {
    ...
    return (<svg width={size} height={size} className="shrink-0 -rotate-90">
      ...
      <motion.circle cx={size / 2} cy={size / 2} r={radius} fill="none" stroke="#3b82f6" strokeWidth="3" strokeLinecap="round" strokeDasharray={circumference} initial={{ strokeDashoffset: circumference }} animate={{ strokeDashoffset: offset }} transition={{ duration: 1.2, ease: 'easeOut', delay: 0.3 }}/>
    </svg>);
};
/* ── Badge Pill ────────────────────────────────────────────── */
const BadgePill = ({ label }) => {
    ...
    return (<span className={`text-[10px] font-bold uppercase tracking-wider px-2 py-0.5 rounded-full border ${cls}`}>
      {label}
    </span>);
};
export const ModuleCard = ({ title, description, icon: Icon, route, iconBg, badge, activity, progress, delay = 0, }) => {
    const navigate = useNavigate();
    return (<motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.4, delay, ease: [0.25, 0.46, 0.45, 0.94] }} whileHover={{ y: -4, transition: { duration: 0.2, ease: 'easeOut' } }} onClick={() => navigate(route)} className="group relative cursor-pointer">
      <div className={`
        ...
        <div className="flex items-center justify-between mt-5 relative">
          {activity && (<span className="text-[11px] text-slate-400 font-medium truncate max-w-[75%]">{activity}</span>)}
          <motion.div className="ml-auto w-8 h-8 rounded-lg bg-slate-100 group-hover:bg-blue-600 flex items-center justify-center transition-colors duration-200" whileHover={{ scale: 1.1 }} whileTap={{ scale: 0.95 }}>
