...
const QuickAction: React.FC<{
  ...
  onClick: () => void;
  iconColor?: string;
}> = ({ icon: Icon, label, onClick, iconColor = 'text-slate-500' }) => (
  <motion.button
    whileHover={{ x: 2 }}
    whileTap={{ scale: 0.97 }}
    onClick={onClick}
    className="flex items-center justify-between w-full px-3.5 py-2.5 rounded-xl bg-slate-50 hover:bg-blue-50 border border-slate-200 hover:border-blue-200 transition-all duration-200 group"
  >
    ...
    <div className="space-y-4">
      ...
      >
        ...
        <motion.button
          ...
          className="mt-4 w-full py-2 rounded-xl bg-slate-50 hover:bg-blue-50 border border-slate-200 hover:border-blue-300 text-[12px] font-bold text-slate-500 hover:text-blue-600 transition-all duration-200 flex items-center justify-center gap-1.5"
        >
      ...
      >
        ...
        <div className="flex gap-1 mt-3">
          {Array.from({ length: 10 }).map((_, i) => (
            <div
              key={i}
              className={`flex-1 h-1.5 rounded-full transition-all ${i < streak ? 'bg-orange-400' : 'bg-slate-100'}`}
            />
          ))}
        </div>
      </motion.div>
      ...
      <motion.div
        ...
        className="rounded-2xl bg-gradient-to-br from-blue-50 to-indigo-50 border border-blue-200 p-5"
      >
        ...
        <motion.button
          ...
          whileTap={{ scale: 0.98 }}
          onClick={() => navigate('/ai')}
          className="mt-3 flex items-center gap-1.5 text-[12px] font-bold text-blue-600 hover:text-blue-700 transition-colors"
        >
