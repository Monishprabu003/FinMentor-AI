...
export const ResultCard = ({ result, onOpenLevelModal }) => {
      ...
      <div className="p-8 rounded-3xl bg-gradient-to-br from-zinc-900 via-indigo-950 to-zinc-900 border border-zinc-800 text-zinc-100 shadow-2xl text-center space-y-6 relative overflow-hidden">
        ...
        <div className="relative w-36 h-36 mx-auto flex items-center justify-center">
          <svg className="w-full h-full transform -rotate-90" viewBox="0 0 36 36">
            ...
            <motion.path className="text-emerald-400" strokeWidth="3.5" strokeDasharray={`${result.scorePercentage}, 100`} strokeLinecap="round" stroke="currentColor" fill="none" initial={{ strokeDasharray: '0, 100' }} animate={{ strokeDasharray: `${result.scorePercentage}, 100` }} transition={{ duration: 1.5, ease: 'easeOut' }} d="M18 2.0845 a 15.9155 15.9155 0 0 1 0 31.831 a 15.9155 15.9155 0 0 1 0 -31.831"/>
          </svg>
        ...
        <div className="flex flex-wrap items-center justify-center gap-3 pt-2">
          <button onClick={() => navigate(`/dashboard/learning/assessment/${result.assessmentId}/review`)} className="px-6 py-3.5 rounded-2xl bg-blue-600 hover:bg-blue-500 text-white font-black text-xs shadow-lg shadow-blue-600/20 flex items-center gap-2 transition-all hover:scale-102">
            <span>Review All Answers</span>
            <ArrowRight className="w-4 h-4"/>
          </button>

          {onOpenLevelModal && (<button onClick={onOpenLevelModal} className="px-5 py-3.5 rounded-2xl bg-zinc-900 hover:bg-zinc-800 border border-zinc-800 text-amber-300 font-extrabold text-xs flex items-center gap-1.5 transition-all">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        ...
        <div className="p-5 rounded-3xl bg-zinc-900 border border-zinc-800 space-y-3">
          ...
          <div className="flex flex-wrap gap-1.5">
            {result.strongTopics.map((topic, i) => (<span key={i} className="text-xs font-bold px-3 py-1 rounded-full bg-emerald-950/60 text-emerald-300 border border-emerald-500/30">
                ✓ {topic}
              </span>))}
          </div>
        </div>
        ...
        <div className="p-5 rounded-3xl bg-zinc-900 border border-zinc-800 space-y-3">
          ...
          <div className="flex flex-wrap gap-1.5">
            {result.weakTopics.map((topic, i) => (<span key={i} className="text-xs font-bold px-3 py-1 rounded-full bg-amber-950/60 text-amber-300 border border-amber-500/30">
                ⚠ {topic}
              </span>))}
          </div>
        </div>
