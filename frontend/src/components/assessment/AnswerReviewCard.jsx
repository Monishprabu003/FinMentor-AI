...
export const AnswerReviewCard = ({ question, selectedOptionId }) => {
    const correctOption = question.options.find((o) => o.isCorrect);
    const selectedOption = question.options.find((o) => o.id === selectedOptionId);
    const isCorrect = selectedOption?.isCorrect ?? false;
    return (<div className="p-6 rounded-3xl bg-zinc-900 border border-zinc-800 text-zinc-100 space-y-4 shadow-md">
      {/* Header */}
      <div className="flex items-center justify-between">
        ...
        {isCorrect ? (<span className="flex items-center gap-1 text-xs font-black text-emerald-400 bg-emerald-950/60 border border-emerald-500/30 px-3 py-1 rounded-full">
            <CheckCircle2 className="w-3.5 h-3.5"/> Correct (+{question.xpReward} XP)
          </span>) : (<span className="flex items-center gap-1 text-xs font-black text-rose-400 bg-rose-950/60 border border-rose-500/30 px-3 py-1 rounded-full">
      ...
      <div className="space-y-2">
        {question.options.map((opt) => {
            ...
            return (<div key={opt.id} className={`p-3.5 rounded-xl border text-xs flex items-center justify-between ${style}`}>
              <div className="flex items-center gap-2">
                <span>{opt.text}</span>
      ...
      <div className="p-4 rounded-2xl bg-zinc-950 border border-zinc-800/80 space-y-2">
        ...
        {question.referenceLessonTitle && (<div className="pt-2 border-t border-zinc-800/60 flex items-center justify-between text-xs font-bold text-indigo-300">
