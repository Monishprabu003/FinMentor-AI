...
export const QuestionCard = ({ question, selectedOptionId, onSelectOption, }) => {
    return (<motion.div key={question.id} initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.3 }} className="p-6 md:p-8 rounded-3xl bg-zinc-900 border border-zinc-800 text-zinc-100 shadow-xl space-y-6">
      ...
      {question.scenarioText && (<div className="p-4 rounded-2xl bg-zinc-950/90 border border-zinc-800/80 space-y-1">
      ...
      <div className="space-y-3 pt-2">
        {question.options.map((opt, idx) => {
            ...
            return (<motion.button key={opt.id} whileHover={{ scale: 1.01 }} whileTap={{ scale: 0.99 }} onClick={() => onSelectOption(opt.id)} className={`w-full p-4 rounded-2xl border text-left flex items-center justify-between text-xs md:text-sm font-semibold transition-all ${isSelected
              ...
              <div className="flex items-center gap-3">
                <span className={`w-6 h-6 rounded-full border flex items-center justify-center text-xs font-black shrink-0 ${isSelected
