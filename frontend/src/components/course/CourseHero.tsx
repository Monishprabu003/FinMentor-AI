...
export const CourseHero: React.FC<CourseHeroProps> = ({ course }) => {
  ...
  return (
    <div className={`relative rounded-3xl bg-gradient-to-br ${course.thumbnailGradient} text-white p-6 md:p-10 shadow-xl overflow-hidden mb-8`}>
        ...
        <div className="lg:col-span-2 space-y-4">
          <div className="flex flex-wrap items-center gap-2">
            ...
            {course.hasCertificate && (
              <span className="text-[11px] font-bold text-amber-300 bg-amber-400/20 border border-amber-400/30 px-3 py-1 rounded-full flex items-center gap-1">
          ...
          <div className="flex flex-wrap items-center gap-3 pt-4">
            <button
              onClick={() => navigate(`/dashboard/learning/course/${course.id}/lesson/${firstLessonId}`)}
              className="px-8 py-4 rounded-2xl bg-white hover:bg-slate-100 text-slate-900 font-black text-sm shadow-xl flex items-center gap-2.5 transition-all hover:scale-[1.02]"
            >
            ...
            <button
              onClick={handleShare}
              className="p-3.5 rounded-2xl bg-black/30 backdrop-blur-md border border-white/20 text-white hover:bg-black/40 transition-all relative"
              title="Share Course"
            >
              <Share2 className="w-5 h-5" />
              {copied && (
                <span className="absolute -top-8 left-1/2 -translate-x-1/2 text-[10px] font-extrabold bg-black text-white px-2 py-0.5 rounded shadow">
                  Link Copied!
                </span>
              )}
            </button>
        ...
        <div className="p-6 rounded-3xl bg-black/30 border border-white/15 backdrop-blur-md flex flex-col items-center justify-center text-center space-y-4">
          ...
          <div className="relative w-32 h-32 flex items-center justify-center">
            <svg className="w-full h-full transform -rotate-90" viewBox="0 0 36 36">
              ...
              <motion.path
                ...
                strokeWidth="3.5"
                strokeDasharray={`${course.progressPercentage}, 100`}
                strokeLinecap="round"
                stroke="currentColor"
                fill="none"
                initial={{ strokeDasharray: '0, 100' }}
                animate={{ strokeDasharray: `${course.progressPercentage}, 100` }}
          ...
          <div className="text-xs text-slate-300 font-medium">
            {course.progressPercentage === 100 ? (
              ...
              <span>2 of 8 lessons completed</span>
            )}
          </div>
        </div>
      </div>
