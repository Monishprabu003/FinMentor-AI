...
export const ReviewsSection = ({ rating, ratingCount, reviews }) => {
    const [helpfulCounts, setHelpfulCounts] = useState({});
    const handleHelpful = (id, initialCount) => {
        setHelpfulCounts((prev) => ({
            ...prev,
            [id]: (prev[id] ?? initialCount) + 1,
        }));
    };
    return (<div className="p-6 rounded-3xl bg-white border border-slate-200 shadow-sm space-y-6 mb-8">
      ...
      <div className="space-y-4">
        {reviews.map((rev) => (<div key={rev.id} className="p-4 rounded-2xl bg-slate-50 border border-slate-100 space-y-2">
            <div className="flex items-center justify-between">
              ...
              <div className="flex items-center gap-0.5">
                {Array.from({ length: rev.rating }).map((_, i) => (<Star key={i} className="w-3 h-3 text-amber-400 fill-amber-400"/>))}
              </div>
            </div>
            ...
            <div className="flex justify-end">
              <button onClick={() => handleHelpful(rev.id, rev.helpfulCount)} className="text-[11px] font-bold text-slate-500 hover:text-blue-600 flex items-center gap-1 transition-colors">
