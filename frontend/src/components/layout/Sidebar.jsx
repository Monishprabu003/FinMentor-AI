...
import { LayoutDashboard, Receipt, PieChart, Target, GraduationCap, BookOpen, Bot, Settings, HelpCircle } from 'lucide-react';
...
export const Sidebar = () => {
    ...
    return (<aside className="w-64 border-r border-slate-200 bg-white/70 backdrop-blur-md flex flex-col justify-between hidden md:flex min-h-[calc(100vh-4rem)]">
      <div className="p-4 space-y-1.5">
        ...
        {navItems.map((item) => {
            ...
            return (<NavLink key={item.path} to={item.path} className={({ isActive }) => clsx('flex items-center gap-3 px-3.5 py-2.5 rounded-xl text-sm font-semibold transition-all duration-200', isActive
                    ? 'bg-blue-600 text-white shadow-md shadow-blue-600/20'
                    : 'text-slate-600 hover:text-slate-900 hover:bg-slate-100/80')}>
              <Icon className="w-4 h-4 shrink-0"/>
              <span>{item.name}</span>
              {item.highlight && (<span className="ml-auto text-[10px] font-bold px-2 py-0.5 rounded-full bg-blue-100 text-blue-700">
                  AI
                </span>)}
            </NavLink>);
        })}
