...
const NavGroupLabel: React.FC<{ label: string; collapsed: boolean }> = ({ label, collapsed }) => (
  <AnimatePresence>
    {!collapsed && (
      <motion.p
        initial={{ opacity: 0 }}
        ...
        className="px-3 pt-4 pb-1 text-[10px] font-black uppercase tracking-[0.12em] text-slate-400 select-none"
      >
        {label}
      </motion.p>
    )}
...
export const AppSidebar: React.FC = () => {
  ...
  return (
    <motion.aside
      ...
      className="hidden md:flex flex-col h-[calc(100vh-56px)] sticky top-14 shrink-0 bg-white border-r border-slate-200 overflow-hidden"
    >
      {/* Collapse Toggle */}
      <div className={`flex ${collapsed ? 'justify-center' : 'justify-end'} px-3 pt-3 pb-2`}>
        <motion.button
          ...
          className="w-6 h-6 rounded-lg bg-slate-100 hover:bg-slate-200 border border-slate-200 flex items-center justify-center transition-colors"
        >
          {collapsed
      ...
      {user && (
        <div className="px-2 py-3 border-t border-slate-200 shrink-0">
          <motion.button
            ...
            className={`flex items-center rounded-xl bg-slate-50 hover:bg-slate-100 border border-slate-200 transition-all duration-200 overflow-hidden ${
              collapsed ? 'w-10 h-10 mx-auto justify-center p-0' : 'gap-2.5 w-full p-2.5'
            }`}
          >
