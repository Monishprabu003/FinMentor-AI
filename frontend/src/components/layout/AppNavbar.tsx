...
const NotificationDropdown: React.FC<{ onClose: () => void }> = ({ onClose }) => (
  <motion.div
    ...
    className="absolute right-0 top-full mt-3 w-80 rounded-2xl bg-white border border-slate-200 shadow-xl shadow-slate-200/60 overflow-hidden z-50"
  >
    ...
    {NOTIFICATIONS.map((n) => (
      <div
        key={n.id}
        className="flex gap-3 px-4 py-3.5 hover:bg-slate-50 transition-colors border-b border-slate-50 last:border-0 cursor-pointer"
      >
...
const AvatarDropdown: React.FC<{
  user: { full_name: string; email: string; experience_level: string };
  onLogout: () => void;
  onSettings: () => void;
  ...
  <motion.div
    ...
    exit={{ opacity: 0, y: -8, scale: 0.96 }}
    transition={{ duration: 0.18, ease: 'easeOut' }}
    className="absolute right-0 top-full mt-3 w-64 rounded-2xl bg-white border border-slate-200 shadow-xl shadow-slate-200/60 overflow-hidden z-50"
  >
    {/* User info */}
    ...
    <div className="p-2">
      <button
        onClick={onSettings}
        className="flex items-center gap-2.5 w-full px-3 py-2.5 rounded-xl hover:bg-slate-50 transition-colors group"
      >
      ...
      <button
        onClick={onLogout}
        className="flex items-center gap-2.5 w-full px-3 py-2.5 rounded-xl hover:bg-rose-50 transition-colors group"
      >
...
export const AppNavbar: React.FC = () => {
  ...
  return (
    <header className="h-14 sticky top-0 z-50 flex items-center px-4 gap-3 bg-white/90 backdrop-blur-xl border-b border-slate-200 shadow-sm">
      ...
      <div className="flex-1 max-w-sm mx-auto lg:mx-4">
        <motion.div animate={{ scale: searchFocused ? 1.01 : 1 }} className="relative flex items-center">
          ...
          <input
            ...
            className={`w-full pl-9 pr-14 py-2 rounded-xl bg-slate-50 border text-[13px] font-medium text-slate-700 placeholder:text-slate-400 outline-none transition-all duration-200 ${
              searchFocused
      ...
      <div className="flex items-center gap-2 shrink-0">
        ...
        <motion.button
          ...
          className="hidden md:flex items-center gap-1.5 px-3 py-1.5 rounded-xl bg-slate-50 hover:bg-slate-100 border border-slate-200 text-[12px] font-bold text-slate-600 hover:text-slate-800 transition-all duration-200"
        >
        ...
        <motion.button
          ...
          whileTap={{ scale: 0.97 }}
          onClick={() => navigate('/ai')}
          className="hidden sm:flex items-center gap-1.5 px-3 py-1.5 rounded-xl bg-blue-50 hover:bg-blue-100 border border-blue-200 text-[12px] font-bold text-blue-600 hover:text-blue-700 transition-all duration-200"
        >
        ...
        <div ref={notifRef} className="relative">
          <motion.button
            ...
            className="relative w-8 h-8 rounded-xl bg-slate-50 hover:bg-slate-100 border border-slate-200 flex items-center justify-center transition-all"
          >
        ...
        {user && (
          <div ref={avatarRef} className="relative">
            <motion.button
              ...
              className="flex items-center gap-2 pl-1 pr-2.5 py-1 rounded-xl bg-slate-50 hover:bg-slate-100 border border-slate-200 hover:border-slate-300 transition-all duration-200"
            >
            ...
            <AnimatePresence>
              {showAvatar && (
                <AvatarDropdown
                  ...
                  onSettings={() => { navigate('/settings'); setShowAvatar(false); }}
                />
              )}
            </AnimatePresence>
          </div>
