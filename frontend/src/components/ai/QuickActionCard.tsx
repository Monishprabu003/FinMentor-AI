...
interface QuickActionCardProps {
  title: string;
  subtitle: string;
  icon: 'pie' | 'chart' | 'calc' | 'help';
  onClick: () => void;
}

export const QuickActionCard: React.FC<QuickActionCardProps> = ({ title, subtitle, icon, onClick }) => {
  const icons = {
    pie: PieChart,
    chart: TrendingUp,
    calc: Calculator,
  ...
  return (
    <motion.div
      whileHover={{ y: -2 }}
      onClick={onClick}
      className="p-4 rounded-2xl bg-white border border-slate-200 shadow-2xs hover:border-blue-300 hover:shadow-md transition-all cursor-pointer flex items-center justify-between group"
    >
