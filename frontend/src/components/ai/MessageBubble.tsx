...
export const MessageBubble: React.FC<MessageBubbleProps> = ({ message, onSelectPrompt }) => {
  ...
  return (
    <div className={`flex gap-3 max-w-3xl ${isAI ? 'mr-auto' : 'ml-auto flex-row-reverse'}`}>
      {/* Avatar Icon */}
      <div
      ...
      {/* Bubble Box */}
      <div
        className={`p-4 md:p-5 rounded-2xl text-xs md:text-sm font-medium leading-relaxed space-y-3 shadow-xs ${
          isAI
      ...
      >
            ...
            <table className="w-full text-left text-xs font-medium border-collapse">
              <thead>
                <tr className="bg-zinc-900 border-b border-zinc-800 text-zinc-300 font-bold">
                  {message.tableData.headers.map((h, idx) => (
        ...
        {message.formulaCode && (
          <div className="p-3.5 rounded-xl bg-zinc-950 border border-zinc-800 font-mono text-xs text-indigo-300 overflow-x-auto">
            <code>{message.formulaCode}</code>
          </div>
        )}

        {/* Follow-up Prompts if present */}
        {isAI && message.followUpPrompts && message.followUpPrompts.length > 0 && (
          <div className="pt-2 border-t border-zinc-800/80 space-y-2">
            ...
            <div className="flex flex-wrap gap-1.5">
              {message.followUpPrompts.map((p, idx) => (
                <button
                  key={idx}
                  onClick={() => onSelectPrompt && onSelectPrompt(p)}
                  className="text-[11px] font-semibold text-blue-400 bg-zinc-950 hover:bg-zinc-800 border border-zinc-800 px-3 py-1 rounded-lg transition-colors text-left"
                >
                  💡 {p}
                </button>
              ))}
        ...
        {/* Actions bar */}
        {isAI && (
          <div className="flex items-center justify-between pt-2 text-[11px] font-bold text-zinc-500 border-t border-zinc-800/60">
