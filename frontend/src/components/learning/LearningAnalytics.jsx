                ...
                : day.intensity === 3
                    ...
                    : day.intensity === 2
                        ...
                        : day.intensity === 1
                            ? 'bg-blue-200'
                            : 'bg-slate-200/60';
            return (<motion.div key={idx} whileHover={{ scale: 1.3 }} title={`${day.date}: ${day.count} lessons completed`} className={`w-3.5 h-3.5 rounded-xs ${bgClass} transition-all cursor-pointer`}/>);
        })}
        </div>
      </div>
    </div>);
