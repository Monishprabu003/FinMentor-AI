...
import { LayoutDashboard, Compass, GraduationCap, BookOpen, Bookmark, Award, Trophy, Zap, BarChart2, Settings, ChevronLeft, ChevronRight, } from 'lucide-react';
const SIDEBAR_ITEMS = [
...
export const LearningSidebar = () => {
    ...
    return (<motion.aside animate={{ width: collapsed ? 68 : 230 }} transition={{ duration: 0.25, ease: [0.25, 0.46, 0.45, 0.94] }} className="hidden md:flex flex-col h-[calc(100vh-56px)] sticky top-14 shrink-0 bg-white border-r border-slate-200 shadow-sm overflow-hidden z-20">
      {/* Header & Toggle */}
      <div className="flex items-center justify-between px-3 pt-3 pb-2 border-b border-slate-100">
        <AnimatePresence>
          {!collapsed && (<motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="flex items-center gap-2 px-1">
        ...
        <motion.button whileHover={{ scale: 1.08 }} whileTap={{ scale: 0.93 }} onClick={() => setCollapsed((v) => !v)} className="w-6 h-6 rounded-lg bg-slate-100 hover:bg-slate-200 border border-slate-200 flex items-center justify-center transition-colors ml-auto">
      ...
      <nav className="flex-1 overflow-y-auto overflow-x-hidden px-2 py-3 space-y-1">
        {SIDEBAR_ITEMS.map((item) => {
            ...
            return (<NavLink key={item.path} to={item.path} end={item.path === '/dashboard/learning'} title={collapsed ? item.label : undefined} className={({ isActive }) => [
              ...
              {({ isActive }) => (<>
                  ...
                  <AnimatePresence initial={false}>
                    {!collapsed && (<motion.span initial={{ opacity: 0, width: 0 }} animate={{ opacity: 1, width: 'auto' }} exit={{ opacity: 0, width: 0 }} transition={{ duration: 0.2 }} className="text-[13px] font-semibold whitespace-nowrap overflow-hidden">
                        {item.label}
                      </motion.span>)}
                  </AnimatePresence>

                  {!collapsed && item.badge && (<span className={[
      ...
      {/* Footer Banner */}
      {!collapsed && (<div className="p-3 m-2 rounded-xl bg-gradient-to-br from-blue-50 to-indigo-50 border border-blue-100 text-xs">
