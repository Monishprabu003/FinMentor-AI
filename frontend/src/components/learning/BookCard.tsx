...
import type { ResourceItem } from '../../types/learning';
...
export const BookCard: React.FC<BookCardProps> = ({ item }) => {
  ...
  return (
    <motion.div
      whileHover={{ y: -4 }}
      className="p-5 rounded-2xl bg-white border border-slate-200 shadow-2xs hover:shadow-md hover:border-blue-300 transition-all duration-200 flex flex-col justify-between"
    >
      <div>
        {/* Book Cover simulation */}
        <div
          className={`h-36 rounded-xl bg-gradient-to-br ${item.coverGradient} text-white p-4 flex flex-col justify-between mb-4 shadow-md relative overflow-hidden`}
        >
        ...
        <div className="space-y-1.5 mb-4">
          ...
          {item.tagline && (
            <p className="text-xs font-semibold text-blue-600 line-clamp-1">
              "{item.tagline}"
            </p>
          )}
