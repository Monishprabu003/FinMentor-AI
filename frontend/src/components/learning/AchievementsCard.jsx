    ...
    return (<div className="p-6 rounded-3xl bg-white border border-slate-200 shadow-sm space-y-4 mb-8">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-5 gap-3">
        {USER_BADGES.map((badge) => {
            ...
            return (<motion.div key={badge.id} whileHover={{ y: -3 }} className={`p-4 rounded-2xl border text-center flex flex-col items-center justify-between transition-all ${badge.unlocked
                    ? 'bg-gradient-to-b from-amber-50/70 to-orange-50/40 border-amber-200 shadow-xs'
                    : 'bg-slate-50 border-slate-200 opacity-60'}`}>
              <div className={`w-12 h-12 rounded-2xl flex items-center justify-center mb-2 shadow-xs ${badge.unlocked
                    ? 'bg-gradient-to-br from-amber-400 to-orange-500 text-white'
                    : 'bg-slate-200 text-slate-400'}`}>
                {badge.unlocked ? (<IconComponent className="w-6 h-6"/>) : (<Lock className="w-5 h-5"/>)}
              </div>
              ...
              {badge.unlocked && badge.unlockedDate && (<span className="text-[9.5px] font-bold text-amber-700 bg-amber-100 px-2 py-0.5 rounded-full mt-2">
                  {badge.unlockedDate}
                </span>)}
            </motion.div>);
        })}
