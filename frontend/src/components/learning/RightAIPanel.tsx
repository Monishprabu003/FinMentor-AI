...
export const RightAIPanel: React.FC = () => {
  ...
  return (
    <div className="w-[300px] shrink-0 border-l border-slate-200 bg-slate-50/70 p-4 space-y-4 hidden xl:block min-h-[calc(100vh-3.5rem)] overflow-y-auto">
      {/* Header Badge */}
      ...
      <motion.div
        whileHover={{ y: -2 }}
        className="p-4 rounded-2xl bg-gradient-to-br from-blue-600 to-indigo-700 text-white shadow-md space-y-2 relative overflow-hidden"
      >
        ...
        <button
          ...
          className="mt-2 w-full py-2 rounded-xl bg-white text-blue-700 text-xs font-extrabold flex items-center justify-center gap-1.5 hover:bg-blue-50 transition-colors shadow-xs"
        >
      ...
      <div className="p-4 rounded-2xl bg-white border border-slate-200 shadow-xs space-y-2.5">
        ...
        <div className="space-y-1.5">
          {MOCK_USER_STATS.weakTopics.map((topic, i) => (
            <div
              key={i}
              className="flex items-center justify-between p-2 rounded-xl bg-amber-50/60 border border-amber-100 text-xs font-semibold text-amber-900"
            >
              <span>{topic}</span>
      ...
      <div className="p-4 rounded-2xl bg-white border border-slate-200 shadow-xs space-y-2.5">
        ...
        <div className="flex flex-wrap gap-1.5">
          {MOCK_USER_STATS.strongTopics.map((topic, i) => (
            <span
              key={i}
              className="text-[10.5px] font-bold px-2.5 py-1 rounded-full bg-emerald-50 text-emerald-700 border border-emerald-200"
            >
              ✓ {topic}
            </span>
          ))}
