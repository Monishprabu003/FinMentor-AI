...
import type { DailyChallenge } from '../../types/learning';
  ...
  return (
    ...
    >
      ...
      <div className="flex items-center justify-between pt-3 border-t border-slate-100">
        ...
        {completed ? (
          <span className="flex items-center gap-1 text-xs font-extrabold text-emerald-600 bg-emerald-100 px-3 py-1.5 rounded-xl">
            <CheckCircle2 className="w-4 h-4" /> Claimed!
          </span>
        ) : (
          <button
            onClick={handleClaim}
            className="px-4 py-2 rounded-xl bg-blue-600 hover:bg-blue-700 text-white font-extrabold text-xs shadow-xs transition-all hover:scale-[1.03]"
          >
            Complete Quest
          </button>
        )}
