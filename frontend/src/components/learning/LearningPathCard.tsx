...
export const LearningPathCard: React.FC<LearningPathCardProps> = ({ path }) => {
  ...
  return (
    <motion.div
      whileHover={{ y: -4 }}
      onClick={() => {
        if (path.unlocked) navigate(`/dashboard/learning/path/${path.id}`);
      }}
      className={`relative p-5 rounded-2xl border transition-all duration-200 shadow-2xs flex flex-col justify-between ${
        path.unlocked
          ...
          <div className="w-full h-2 bg-slate-100 rounded-full overflow-hidden">
            <div
              className={`h-full rounded-full transition-all duration-500 ${
                path.completionPercentage === 100 ? 'bg-emerald-500' : 'bg-blue-600'
              }`}
