      ...
      {isOpen && (
        ...
        >
          ...
          <div className="flex-1 overflow-y-auto p-4 space-y-3 scrollbar-thin scrollbar-thumb-zinc-800">
            {notes.map((note, idx) => (
              <div key={idx} className="p-3.5 rounded-2xl bg-zinc-900 border border-zinc-800 space-y-2 relative group">
          ...
          <div className="p-4 border-t border-zinc-800 bg-zinc-900/80 space-y-2">
            <textarea
              ...
              placeholder="Write a personal note or key insight..."
              rows={2}
              className="w-full p-3 rounded-xl bg-zinc-950 border border-zinc-800 text-xs font-medium text-zinc-100 placeholder-zinc-500 focus:outline-none focus:border-indigo-500 resize-none"
            />
            <div className="flex items-center justify-between">
              <button
                onClick={() => alert('Notes exported to Markdown!')}
                className="text-[11px] font-bold text-zinc-400 hover:text-zinc-200 flex items-center gap-1 transition-colors"
              >
                <Download className="w-3.5 h-3.5" /> Export Notes
              </button>
              <button
                onClick={handleAdd}
                className="px-4 py-2 rounded-xl bg-indigo-600 hover:bg-indigo-500 text-white text-xs font-extrabold flex items-center gap-1 shadow-xs transition-all"
              >
