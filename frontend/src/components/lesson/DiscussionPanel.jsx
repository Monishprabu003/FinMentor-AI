...
export const DiscussionPanel = () => {
    ...
    return (<div className="p-6 md:p-8 rounded-3xl bg-zinc-900 border border-zinc-800 text-zinc-100 space-y-6 my-8 shadow-xl">
      ...
      <div className="flex gap-2">
        <input type="text" value={newComment} onChange={(e) => setNewComment(e.target.value)} onKeyDown={(e) => e.key === 'Enter' && handlePost()} placeholder="Ask a question about this lesson..." className="flex-1 px-4 py-2.5 rounded-xl bg-zinc-950 border border-zinc-800 text-xs font-medium text-zinc-100 placeholder-zinc-500 focus:outline-none focus:border-blue-500"/>
        <button onClick={handlePost} className="px-4 py-2.5 rounded-xl bg-blue-600 hover:bg-blue-500 text-white font-extrabold text-xs shadow-xs flex items-center gap-1.5 transition-all">
      ...
      <div className="space-y-4 pt-2">
        {comments.map((c) => (<div key={c.id} className="p-4 rounded-2xl bg-zinc-950 border border-zinc-800/80 space-y-3">
            ...
            {c.instructorReply && (<div className="p-3.5 rounded-xl bg-blue-950/40 border border-blue-500/30 text-xs space-y-1">
