...
export const AITutorPanel = ({ isOpen, onClose, lessonTitle }) => {
    ...
    return (<AnimatePresence>
      {isOpen && (<motion.div initial={{ x: 380, opacity: 0 }} animate={{ x: 0, opacity: 1 }} exit={{ x: 380, opacity: 0 }} transition={{ duration: 0.25, ease: [0.25, 0.46, 0.45, 0.94] }} className="fixed right-0 top-14 bottom-0 w-96 bg-zinc-950 border-l border-zinc-800 text-zinc-100 shadow-2xl z-50 flex flex-col justify-between">
          ...
          <div className="flex-1 overflow-y-auto p-4 space-y-3 scrollbar-thin scrollbar-thumb-zinc-800">
            {messages.map((m, idx) => (<div key={idx} className={`p-3.5 rounded-2xl text-xs font-medium leading-relaxed max-w-[85%] ${m.sender === 'user'
          ...
          <div className="p-3 border-t border-zinc-800 bg-zinc-900/40 space-y-2">
            ...
            <div className="flex flex-wrap gap-1.5">
              <button onClick={() => handleQuickPrompt('Simplify the 50/30/20 rule in simple terms')} className="px-2.5 py-1 rounded-lg bg-zinc-900 hover:bg-zinc-800 border border-zinc-800 text-[11px] font-semibold text-zinc-300 transition-colors text-left">
                💡 Simplify 50/30/20
              </button>
              <button onClick={() => handleQuickPrompt('Give me a real-life example of lifestyle inflation')} className="px-2.5 py-1 rounded-lg bg-zinc-900 hover:bg-zinc-800 border border-zinc-800 text-[11px] font-semibold text-zinc-300 transition-colors text-left">
          ...
          <div className="p-3 border-t border-zinc-800 bg-zinc-900/80 flex items-center gap-2">
            <input type="text" value={input} onChange={(e) => setInput(e.target.value)} onKeyDown={(e) => e.key === 'Enter' && handleSend()} placeholder="Ask AI Tutor anything..." className="flex-1 px-3.5 py-2 rounded-xl bg-zinc-950 border border-zinc-800 text-xs font-medium text-zinc-100 placeholder-zinc-500 focus:outline-none focus:border-blue-500"/>
            <button onClick={handleSend} className="p-2 rounded-xl bg-blue-600 hover:bg-blue-500 text-white transition-all shadow-xs">
