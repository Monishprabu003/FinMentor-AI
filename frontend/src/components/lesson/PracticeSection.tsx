...
export const PracticeSection: React.FC<PracticeSectionProps> = ({ questions, onCompletePractice }) => {
    ...
    <div className="p-6 md:p-8 rounded-3xl bg-zinc-900 border border-zinc-800 text-zinc-100 space-y-6 my-8 shadow-xl">
      ...
      <div className="space-y-6">
        {questions.map((q, qIdx) => {
          ...
          return (
            <div key={q.id} className="p-5 rounded-2xl bg-zinc-950 border border-zinc-800 space-y-4">
              <div className="flex items-center justify-between">
              ...
              <div className="space-y-2">
                {q.options.map((opt, optIdx) => {
                  ...
                  return (
                    <motion.button
                      key={optIdx}
                      whileHover={{ x: isSubmitted ? 0 : 2 }}
                      onClick={() => handleSelect(q.id, optIdx)}
                      className={`w-full p-3.5 rounded-xl border text-left text-xs font-semibold flex items-center justify-between transition-all ${btnStyle}`}
                    >
              ...
              {!isSubmitted ? (
                ...
                <motion.div initial={{ opacity: 0, y: 4 }} animate={{ opacity: 1, y: 0 }} className="p-3.5 rounded-xl bg-zinc-900 border border-zinc-800 text-xs text-zinc-300 space-y-1">
