...
export const BooksLibraryPage = () => {
    ...
    return (<div className="p-6 md:p-10 max-w-7xl mx-auto space-y-8">
      ...
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
        ...
        <button onClick={() => navigate('/ai')} className="px-5 py-3 rounded-2xl bg-blue-600 hover:bg-blue-700 text-white font-bold text-xs shadow-lg shadow-blue-600/20 hover:shadow-xl hover:shadow-blue-600/30 transition-all flex items-center gap-2">
          <Sparkles className="w-4 h-4 text-white"/>
          <span>Ask AI to Summarize a Book</span>
        </button>
      </div>

      {loading ? (<div className="p-12 text-center text-slate-500 font-semibold text-sm">Loading book library...</div>) : (<div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          {books.map((b) => (<div key={b.id} className="bg-white/90 backdrop-blur-xl p-7 sm:p-8 rounded-[2rem] border border-slate-200/80 shadow-xl shadow-slate-200/40 space-y-6 flex flex-col justify-between hover:border-blue-300 transition-all">
              <div className="space-y-4">
                ...
                <div className="space-y-2.5 pt-2">
                  ...
                  <ul className="space-y-2">
                    {b.key_takeaways.map((takeaway, i) => (<li key={i} className="text-xs text-slate-600 font-medium flex items-start gap-2.5">
              ...
              <div className="p-5 rounded-2xl bg-blue-50/60 border border-blue-100 space-y-3">
                ...
                <ul className="space-y-2">
                  {b.action_checklist.map((item, i) => (<li key={i} className="flex items-start gap-2.5 text-xs text-slate-700 font-medium">
