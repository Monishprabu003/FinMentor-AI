...
export const LeaderboardHubPage = () => {
      ...
      <div className="max-w-4xl mx-auto space-y-6 pb-12">
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
          ...
          <div className="flex items-center gap-2">
            {['Global', 'Weekly', 'Friends'].map((t) => (<button key={t} onClick={() => setTab(t)} className={`px-3.5 py-1.5 rounded-full text-xs font-extrabold transition-all ${tab === t
        ...
        <div className="p-6 rounded-3xl bg-white border border-slate-200 shadow-sm space-y-4">
          ...
          <div className="space-y-2">
            {LEADERBOARD_USERS.map((user) => (<div key={user.id} className={`p-4 rounded-2xl border flex items-center justify-between text-xs font-bold transition-all ${user.isCurrentUser
                ? 'bg-blue-50 border-blue-300 shadow-xs'
                : 'bg-white border-slate-100 hover:bg-slate-50'}`}>
                <div className="flex items-center gap-3">
                  <span className={`w-7 h-7 rounded-full flex items-center justify-center font-black text-xs ${user.rank === 1
                ...
                : user.rank === 2
                  ...
                  <div>
                    <p className="text-sm font-black text-slate-900 flex items-center gap-2">
                      <span>{user.name}</span>
                      {user.isCurrentUser && (<span className="text-[10px] font-extrabold px-2 py-0.5 rounded-full bg-blue-600 text-white">
                          YOU
                        </span>)}
                    </p>
