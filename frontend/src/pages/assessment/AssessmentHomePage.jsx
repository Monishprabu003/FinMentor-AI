...
export const AssessmentHomePage = () => {
    ...
    const filteredAssessments = MOCK_ASSESSMENTS.filter((asm) => {
        if (filter === 'All')
            return true;
        if (filter === 'Completed')
            return (asm.userBestScorePercentage || 0) > 0;
      ...
      <div className="max-w-6xl mx-auto space-y-8 pb-12">
        ...
        <div className="flex items-center justify-between gap-4">
          <div className="flex items-center gap-2 overflow-x-auto pb-1 scrollbar-none">
            {['All', 'Completed', 'Beginner', 'Intermediate', 'Advanced'].map((cat) => (<button key={cat} onClick={() => setFilter(cat)} className={`px-3.5 py-1.5 rounded-full text-xs font-extrabold transition-all shrink-0 ${filter === cat
        ...
        <div className="space-y-4">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            {filteredAssessments.map((asm) => (<motion.div key={asm.id} whileHover={{ y: -3 }} className="p-6 rounded-3xl bg-white border border-slate-200 shadow-sm hover:shadow-md hover:border-blue-300 transition-all flex flex-col justify-between">
                <div className="space-y-3">
                  <div className="flex items-center justify-between">
                    ...
                    <div className="flex items-center gap-2">
                      {asm.userBestScorePercentage ? (<span className="text-xs font-extrabold text-emerald-600 bg-emerald-50 px-2.5 py-0.5 rounded-full flex items-center gap-1 border border-emerald-200">
                ...
                <div className="pt-4 mt-4 border-t border-slate-100 flex items-center justify-between">
                  ...
                  <button onClick={() => navigate(`/dashboard/learning/assessment/${asm.id}`)} className="px-5 py-2.5 rounded-2xl bg-blue-600 hover:bg-blue-700 text-white font-extrabold text-xs shadow-md shadow-blue-600/20 flex items-center gap-1.5 transition-all hover:scale-102">
