...
export const AssessmentReviewPage: React.FC = () => {
  ...
  return (
    <div className="min-h-screen bg-[#09090b] text-zinc-100 font-sans p-6 md:p-10">
      <div className="max-w-4xl mx-auto space-y-6">
        <button
          onClick={() => navigate(`/dashboard/learning/assessment/${assessment.id}/result`)}
          className="flex items-center gap-1.5 text-xs font-extrabold text-zinc-400 hover:text-zinc-200 transition-colors"
        >
        ...
        <div className="space-y-6">
          {assessment.questions.map((q) => (
            <AnswerReviewCard
              key={q.id}
              question={q}
              selectedOptionId={result.userAnswers[q.id]}
            />
          ))}
        </div>
      </div>
