...
export const SavingsGoalsPage = () => {
    ...
    return (<div className="p-6 md:p-10 max-w-7xl mx-auto space-y-6">
      ...
      {loading ? (<div className="p-12 text-center text-slate-500 font-semibold text-sm">Loading savings milestones...</div>) : goals.length === 0 ? (<div className="bg-white/90 backdrop-blur-xl p-12 rounded-[2rem] border border-slate-200/80 shadow-xl shadow-slate-200/40 text-center space-y-3">
          ...
          {goals.map((g) => (<div key={g.id} className="bg-white/90 backdrop-blur-xl p-6 sm:p-7 rounded-[2rem] border border-slate-200/80 shadow-xl shadow-slate-200/40 space-y-4 flex flex-col justify-between hover:border-blue-300 transition-all">
              <div>
                ...
                <div className="mt-4 space-y-2">
                  ...
                  <div className="w-full h-3 bg-slate-100 rounded-full overflow-hidden">
                    <div className="h-full bg-gradient-to-r from-blue-600 to-indigo-500 rounded-full transition-all duration-500" style={{ width: `${Math.min(100, g.progress_percentage)}%` }}/>
                  </div>
                </div>
              </div>

              <div className="pt-3 border-t border-slate-100 space-y-1">
                {g.monthly_savings_required ? (<div className="flex items-center justify-between text-xs font-medium">
                    <span className="text-slate-500 flex items-center gap-1.5">
                      <Calendar className="w-3.5 h-3.5 text-blue-600"/>
                      Monthly Contribution Required
                    </span>
                    <span className="font-black text-blue-600">₹{g.monthly_savings_required.toLocaleString('en-IN')}/mo</span>
                  </div>) : (<div className="text-xs font-semibold text-emerald-600 flex items-center gap-1.5">
          ...
          <div className="bg-white max-w-sm w-full p-8 rounded-[2rem] shadow-2xl border border-slate-100 space-y-5">
            ...
            <form onSubmit={handleCreateGoal} className="space-y-4">
              <div>
                ...
                <input type="text" required value={name} onChange={(e) => setName(e.target.value)} placeholder="e.g. 3-Month Emergency Fund" className="w-full px-4 py-3 rounded-2xl bg-slate-50 border border-slate-200 text-sm font-medium text-slate-900 focus:outline-none focus:border-blue-500 focus:ring-4 focus:ring-blue-500/10 transition-all"/>
              </div>

              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label className="text-xs font-bold text-slate-500 uppercase tracking-wider block mb-1.5">Target (₹)</label>
                  <input type="number" required value={targetAmount} onChange={(e) => setTargetAmount(e.target.value)} className="w-full px-4 py-3 rounded-2xl bg-slate-50 border border-slate-200 text-sm font-medium text-slate-900 focus:outline-none focus:border-blue-500 focus:ring-4 focus:ring-blue-500/10 transition-all"/>
                </div>

                <div>
                  <label className="text-xs font-bold text-slate-500 uppercase tracking-wider block mb-1.5">Saved (₹)</label>
                  <input type="number" value={currentAmount} onChange={(e) => setCurrentAmount(e.target.value)} className="w-full px-4 py-3 rounded-2xl bg-slate-50 border border-slate-200 text-sm font-medium text-slate-900 focus:outline-none focus:border-blue-500 focus:ring-4 focus:ring-blue-500/10 transition-all"/>
                </div>
              </div>
              ...
              <div className="grid grid-cols-2 gap-3">
                ...
                <div>
                  ...
                  <input type="date" value={targetDate} onChange={(e) => setTargetDate(e.target.value)} className="w-full px-4 py-3 rounded-2xl bg-slate-50 border border-slate-200 text-sm font-medium text-slate-900 focus:outline-none focus:border-blue-500 transition-all"/>
                </div>
              </div>
