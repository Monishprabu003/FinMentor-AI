      ...
      <div className="h-[calc(100vh-130px)] rounded-3xl bg-zinc-950 border border-zinc-800 text-zinc-100 shadow-2xl flex overflow-hidden">
        ...
        <div className="w-64 border-r border-zinc-800 bg-zinc-900/60 hidden md:flex flex-col justify-between shrink-0">
          ...
          <div className="flex-1 overflow-y-auto p-2 space-y-1 scrollbar-thin scrollbar-thumb-zinc-800">
            {MOCK_CONVERSATIONS.map((conv) => (<button key={conv.id} onClick={() => {
                setActiveConvId(conv.id);
                setMessages(conv.messages);
            }} className={`w-full p-3 rounded-xl text-left text-xs font-semibold transition-all ${activeConvId === conv.id
        ...
        <div className="flex-1 flex flex-col justify-between min-w-0 bg-[#09090b]">
          ...
          <div className="flex-1 overflow-y-auto p-4 md:p-6 space-y-6 scrollbar-thin scrollbar-thumb-zinc-800">
            {messages.map((m) => (<MessageBubble key={m.id} message={m} onSelectPrompt={(pText) => handleSend(pText)}/>))}
          ...
          <div className="p-4 border-t border-zinc-800 bg-zinc-950/90 space-y-3">
        ...
        ].map((chip, idx) => (<button key={idx} onClick={() => handleSend(chip)} className="px-3 py-1 rounded-full bg-zinc-900 hover:bg-zinc-800 border border-zinc-800 text-[11px] font-semibold text-zinc-300 transition-colors shrink-0">
                  💡 {chip}
                </button>))}
            </div>
            ...
            <div className="flex items-center gap-2">
              <input type="text" value={input} onChange={(e) => setInput(e.target.value)} onKeyDown={(e) => e.key === 'Enter' && handleSend()} placeholder="Ask FinMentor AI anything about personal finance, investing, or tax strategies..." className="flex-1 px-4 py-3 rounded-2xl bg-zinc-900 border border-zinc-800 text-xs md:text-sm font-medium text-zinc-100 placeholder-zinc-500 focus:outline-none focus:border-blue-500 shadow-inner"/>
              <button onClick={() => handleSend()} className="p-3.5 rounded-2xl bg-blue-600 hover:bg-blue-500 text-white font-extrabold text-xs shadow-lg shadow-blue-600/20 transition-all hover:scale-105 shrink-0 cursor-pointer">
