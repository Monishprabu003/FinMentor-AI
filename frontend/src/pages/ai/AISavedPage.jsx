...
export const AISavedPage = () => {
    const savedMessages = MOCK_CONVERSATIONS.flatMap((c) => c.messages).filter((m) => m.sender === 'ai');
    return (<AILayout>
      <div className="max-w-4xl mx-auto space-y-6">
        ...
        <div className="space-y-4">
          {savedMessages.map((m) => (<MessageBubble key={m.id} message={{ ...m, bookmarked: true }}/>))}
        </div>
      </div>
    </AILayout>);
};
