...
export const PathDetailPage = () => {
    ...
    return (<LearningLayout>
      <div className="max-w-4xl mx-auto space-y-6 pb-12">
        <button onClick={() => navigate('/dashboard/learning/paths')} className="flex items-center gap-1.5 text-xs font-extrabold text-slate-500 hover:text-slate-800 transition-colors">
        ...
        <div className="p-6 md:p-8 rounded-3xl bg-gradient-to-br from-slate-900 via-indigo-950 to-slate-900 text-white shadow-xl space-y-4 relative overflow-hidden">
          <div className="flex items-center justify-between">
            ...
            {path.hasCertificate && (<span className="text-xs font-bold text-amber-300 bg-amber-400/20 border border-amber-400/30 px-3 py-1 rounded-full flex items-center gap-1">
        ...
        <div className="p-6 rounded-3xl bg-white border border-slate-200 shadow-sm space-y-6">
          ...
          <div className="space-y-4">
            {path.milestones && path.milestones.length > 0 ? (path.milestones.map((node, i) => (<div key={node.id} className={`p-4 rounded-2xl border flex items-center justify-between transition-all ${node.completed
                ? 'bg-emerald-50/50 border-emerald-200'
                : 'bg-white border-slate-200 hover:border-blue-300 shadow-2xs'}`}>
                  <div className="flex items-center gap-3">
                    <div className={`w-9 h-9 rounded-xl flex items-center justify-center font-bold text-sm ${node.completed
                ...
                </div>))) : (<div className="p-8 text-center text-slate-500 font-medium">
