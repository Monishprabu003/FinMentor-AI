...
import { LEARNING_PATHS, POPULAR_COURSES, LIBRARY_RESOURCES, DAILY_CHALLENGES, } from '../../constants/learningData';
export const LearningHomePage = () => {
      ...
      <div className="max-w-6xl mx-auto space-y-8 pb-12">
        ...
        <div className="space-y-4">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
            {LEARNING_PATHS.slice(0, 6).map((path) => (<LearningPathCard key={path.id} path={path}/>))}
          </div>
        </div>
        ...
        <div className="space-y-4 pt-4">
          <div className="flex items-center justify-between">
            ...
            <button onClick={() => navigate('/dashboard/learning/courses')} className="text-xs font-extrabold text-blue-600 hover:text-blue-700 flex items-center gap-1 group">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
            {POPULAR_COURSES.slice(0, 3).map((course) => (<CourseCard key={course.id} course={course}/>))}
          </div>
        </div>
        ...
        <div className="space-y-4 pt-4">
          <div className="flex items-center justify-between">
            ...
            <button onClick={() => navigate('/dashboard/learning/library')} className="text-xs font-extrabold text-blue-600 hover:text-blue-700 flex items-center gap-1 group">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
            {LIBRARY_RESOURCES.slice(0, 3).map((item) => (<BookCard key={item.id} item={item}/>))}
          </div>
        </div>
        ...
        <div className="space-y-4 pt-4">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            {DAILY_CHALLENGES.map((ch) => (<ChallengeCard key={ch.id} challenge={ch}/>))}
          </div>
        </div>
