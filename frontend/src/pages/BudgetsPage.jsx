...
export const BudgetsPage = () => {
    ...
    return (<div className="p-6 md:p-10 max-w-7xl mx-auto space-y-6">
      ...
      {loading ? (<div className="p-12 text-center text-slate-500 font-semibold text-sm">Loading budget guardrails...</div>) : budgets.length === 0 ? (<div className="bg-white/90 backdrop-blur-xl p-12 rounded-[2rem] border border-slate-200/80 shadow-xl shadow-slate-200/40 text-center space-y-3">
          ...
          {budgets.map((b) => (<div key={b.id} className="bg-white/90 backdrop-blur-xl p-6 sm:p-7 rounded-[2rem] border border-slate-200/80 shadow-xl shadow-slate-200/40 space-y-4 hover:border-blue-300 transition-all">
              <div className="flex items-center justify-between">
                <span className="font-black text-slate-900 text-lg tracking-tight">{b.category}</span>
                <span className={`text-xs font-bold px-3 py-1 rounded-full ${b.status === 'OVER_BUDGET'
              ...
              <div className="space-y-1.5">
                ...
                <div className="w-full h-3 bg-slate-100 rounded-full overflow-hidden">
                  <div className={`h-full transition-all duration-500 rounded-full ${b.status === 'OVER_BUDGET'
                    ? 'bg-rose-500'
                    : b.status === 'WARNING'
                        ? 'bg-amber-500'
                        : 'bg-blue-600'}`} style={{ width: `${Math.min(100, b.percent_used)}%` }}/>
                </div>
              </div>
          ...
          <div className="bg-white max-w-sm w-full p-8 rounded-[2rem] shadow-2xl border border-slate-100 space-y-5">
            ...
            <form onSubmit={handleCreateBudget} className="space-y-4">
              ...
              <div>
                ...
                <input type="number" required value={monthlyLimit} onChange={(e) => setMonthlyLimit(e.target.value)} className="w-full px-4 py-3 rounded-2xl bg-slate-50 border border-slate-200 text-sm font-medium text-slate-900 focus:outline-none focus:border-blue-500 focus:ring-4 focus:ring-blue-500/10 transition-all"/>
              </div>
