...
export const AIAssistantPage = () => {
    ...
    const [conceptInput, setConceptInput] = useState('ETF (Exchange-Traded Fund)');
    ...
    return (<div className="p-6 md:p-10 max-w-6xl mx-auto space-y-6">
      {/* Header */}
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
        ...
        <div className="flex items-center gap-1.5 p-1.5 rounded-2xl bg-white border border-slate-200 shadow-sm">
          <button onClick={() => setActiveTab('chat')} className={`px-4 py-2 rounded-xl text-xs font-bold transition-all ${activeTab === 'chat'
            ? 'bg-blue-600 text-white shadow-md shadow-blue-600/20'
            : 'text-slate-600 hover:text-slate-900 hover:bg-slate-50'}`}>
            AI Chat Tutor
          </button>
          <button onClick={() => setActiveTab('explain')} className={`px-4 py-2 rounded-xl text-xs font-bold transition-all ${activeTab === 'explain'
            ? 'bg-blue-600 text-white shadow-md shadow-blue-600/20'
            : 'text-slate-600 hover:text-slate-900 hover:bg-slate-50'}`}>
            Concept Explainer (ELI5)
          </button>
          <button onClick={() => setActiveTab('insights')} className={`px-4 py-2 rounded-xl text-xs font-bold transition-all ${activeTab === 'insights'
            ? 'bg-blue-600 text-white shadow-md shadow-blue-600/20'
            : 'text-slate-600 hover:text-slate-900 hover:bg-slate-50'}`}>
            Spending Insights
          </button>
      ...
      {/* Tab 1: AI Chat Tutor */}
      {activeTab === 'chat' && (<div className="bg-white/90 backdrop-blur-xl rounded-[2rem] border border-slate-200/80 shadow-xl shadow-slate-200/40 flex flex-col h-[640px] overflow-hidden">
          {/* Messages display */}
          <div className="flex-1 overflow-y-auto p-6 md:p-8 space-y-4">
            {messages.map((m, idx) => (<div key={idx} className={`flex gap-3.5 ${m.sender === 'user' ? 'justify-end' : 'justify-start'}`}>
                {m.sender === 'ai' && (<div className="w-9 h-9 rounded-xl bg-blue-600 flex items-center justify-center shrink-0 mt-0.5 shadow-md shadow-blue-600/20">
                    <Bot className="w-5 h-5 text-white"/>
                  </div>)}

                <div className={`max-w-xl p-5 rounded-2xl text-sm leading-relaxed ${m.sender === 'user'
            ...
            {loadingChat && (<div className="flex items-center gap-3 text-slate-500 text-xs font-semibold">
          ...
          <div className="px-6 py-3 bg-slate-50/80 border-t border-slate-100 flex items-center gap-2 overflow-x-auto">
            ...
            {suggestedQuestions.map((q, idx) => (<button key={idx} onClick={() => handleSendMessage(q)} className="px-3.5 py-1.5 rounded-full bg-white hover:bg-blue-50 border border-slate-200 hover:border-blue-200 text-slate-700 hover:text-blue-600 text-xs font-medium shrink-0 transition-all shadow-2xs">
                {q}
              </button>))}
          </div>
          ...
          <div className="p-4 sm:p-5 bg-white border-t border-slate-100">
            <form onSubmit={(e) => {
              ...
              <input type="text" placeholder="Ask any personal finance question (e.g. ETFs, taxes, emergency funds)..." value={inputMessage} onChange={(e) => setInputMessage(e.target.value)} className="flex-1 px-5 py-3.5 rounded-2xl bg-slate-50 border border-slate-200 text-sm font-medium text-slate-900 placeholder:text-slate-400 focus:outline-none focus:border-blue-500 focus:ring-4 focus:ring-blue-500/10 transition-all"/>
              <button type="submit" disabled={loadingChat || !inputMessage.trim()} className="p-3.5 rounded-2xl bg-blue-600 text-white disabled:opacity-40 hover:bg-blue-700 transition-all shadow-md shadow-blue-600/20">
      ...
      {activeTab === 'explain' && (<div className="bg-white/90 backdrop-blur-xl p-8 sm:p-10 rounded-[2rem] border border-slate-200/80 shadow-xl shadow-slate-200/40 space-y-8">
          ...
          <form onSubmit={handleExplainConcept} className="flex gap-3 max-w-xl">
            <input type="text" value={conceptInput} onChange={(e) => setConceptInput(e.target.value)} placeholder="e.g. Compound Interest, Expense Ratio, Asset vs Liability" className="flex-1 px-5 py-3.5 rounded-2xl bg-slate-50 border border-slate-200 text-sm font-medium text-slate-900 focus:outline-none focus:border-blue-500 focus:ring-4 focus:ring-blue-500/10 transition-all"/>
            <button type="submit" disabled={loadingConcept} className="px-6 py-3.5 rounded-2xl bg-blue-600 hover:bg-blue-700 text-white font-bold text-sm shadow-md shadow-blue-600/20 transition-all">
              {loadingConcept ? 'Explaining...' : 'Explain'}
            </button>
          </form>

          {conceptResult && (<div className="p-7 rounded-[1.75rem] bg-slate-50 border border-slate-200/80 space-y-5 max-w-3xl">
      ...
      {activeTab === 'insights' && (<div className="space-y-6">
          ...
          {loadingInsights ? (<div className="p-12 text-center text-slate-500 font-semibold text-sm">Analyzing spending habits...</div>) : insights.length === 0 ? (<div className="p-12 text-center text-slate-400 font-semibold text-sm">
              Log transactions to unlock AI spending habits coaching.
            </div>) : (<div className="grid grid-cols-1 md:grid-cols-2 gap-5">
              {insights.map((ins, idx) => (<div key={idx} className="bg-white/90 backdrop-blur-xl p-6 rounded-[1.75rem] border border-slate-200/80 shadow-xl shadow-slate-200/40 space-y-3">
                  <div className="flex items-center justify-between">
