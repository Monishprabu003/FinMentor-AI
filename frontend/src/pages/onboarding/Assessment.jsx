...
import { Check, ChevronRight, ChevronLeft, Target, Award, ArrowRight, Sparkles, User, Briefcase, IndianRupee, PiggyBank, ShieldCheck, Wallet, BookOpen, BarChart3, Home, Car, Plane, GraduationCap, Rocket, Shield, Brain, Flame, LineChart, Lightbulb, RotateCcw, Eye, EyeOff, CheckCircle2, XCircle, HelpCircle, ArrowLeft } from 'lucide-react';
/* ═══════════════════════════════════════════════════════════════
...
const RISK_DATA = [
    { value: "Sell everything to protect capital (Low Risk)", level: "Conservative", color: "emerald", icon: Shield, desc: "You prefer safety. Capital preservation is your priority.", tag: "Low Risk" },
    { value: "Hold and wait for recovery (Medium Risk)", level: "Balanced", color: "blue", icon: BarChart3, desc: "You stay calm during volatility and trust the long-term trend.", tag: "Medium Risk" },
    { value: "Buy more at a discount (High Risk)", level: "Aggressive", color: "amber", icon: Flame, desc: "You see dips as opportunities. Higher risk for higher rewards.", tag: "High Risk" },
];
const LOADING_STEPS = [
...
const rawNum = (val) => val.replace(/[^0-9]/g, '');
...
const CircularScore = ({ value }) => {
    ...
    return (<div className="relative flex flex-col items-center justify-center">
      <svg height={radius * 2} width={radius * 2} className="rotate-[-90deg]">
        ...
        <motion.circle stroke="#2563eb" fill="transparent" strokeWidth={stroke} strokeDasharray={`${circum} ${circum}`} style={{ strokeDashoffset }} strokeLinecap="round" r={normRad} cx={radius} cy={radius} initial={{ strokeDashoffset: circum }} animate={{ strokeDashoffset }} transition={{ duration: 1.4, ease: [0.22, 0.68, 0.36, 1] }}/>
      </svg>
...
const ContextPanel = ({ step, quizIdx }) => {
    ...
    return (<div className="space-y-6">
      ...
      {step === 3 && (<div className="bg-white rounded-2xl border border-slate-100 p-4 shadow-sm">
          ...
          <div className="h-2 bg-slate-100 rounded-full overflow-hidden">
            <motion.div className="h-full bg-blue-600 rounded-full" animate={{ width: `${((quizIdx + 1) / KNOWLEDGE_QUESTIONS.length) * 100}%` }} transition={{ duration: 0.35 }}/>
          </div>
        </div>)}
...
const PremiumInput = ({ icon: Icon, label, type, value, onChange, placeholder, prefix }) => (<div className="group">
    ...
    <div className="relative">
      ...
      <input type={type} value={value} onChange={e => onChange(e.target.value)} placeholder={placeholder} aria-label={label} className={`w-full ${prefix ? 'pl-14' : 'pl-11'} pr-5 py-4 text-[15px] text-slate-900 font-medium bg-white border-2 border-slate-150 rounded-2xl focus:border-blue-500 focus:ring-4 focus:ring-blue-500/10 outline-none transition-all placeholder:text-slate-300`}/>
    </div>
  </div>);
const HabitCard = ({ label, val, onChange, opts }) => (<div className="bg-white rounded-2xl border border-slate-100 p-6 shadow-sm">
    <h3 className="text-[15px] font-bold text-slate-800 mb-4">{label}</h3>
    <div className="grid grid-cols-1 sm:grid-cols-2 gap-2.5">
      {opts.map(o => {
        const sel = val === o;
        return (<motion.button key={o} whileHover={{ scale: 1.01, y: -1 }} whileTap={{ scale: 0.99 }} onClick={() => onChange(o)} aria-pressed={sel} className={`p-3.5 rounded-xl text-[13.5px] font-medium text-left transition-all duration-200 border-2 ${sel
            ...
            <span className="flex items-center gap-2.5">
              <span className={`w-5 h-5 rounded-md flex items-center justify-center shrink-0 ${sel ? 'bg-white/20' : 'border-2 border-slate-200'}`}>
...
export const AssessmentPage = () => {
    ...
    const [knowledge, setKnowledge] = useState(Array(10).fill(-1));
    ...
    useEffect(() => {
        if (user?.assessment_completed && !summary && !isRetaking) {
            ...
            if (user.assessment_answers) {
                if (Array.isArray(user.assessment_answers.knowledge))
                    setKnowledge(user.assessment_answers.knowledge);
                if (user.assessment_answers.habits)
                    setHabits(user.assessment_answers.habits);
                if (user.assessment_answers.risk)
                    setRisk(user.assessment_answers.risk);
            }
            if (user.monthly_income)
                setPersonal(p => ({ ...p, income: String(user.monthly_income) }));
            if (user.monthly_expenses)
                setPersonal(p => ({ ...p, expenses: String(user.monthly_expenses) }));
            if (user.financial_goals && Array.isArray(user.financial_goals))
                setGoals(user.financial_goals);
        }
    }, [user, summary, isRetaking]);
    const calculateScore = useCallback(() => {
        let s = 0;
        knowledge.forEach((a, i) => { if (a === KNOWLEDGE_QUESTIONS[i].correct)
            s += 10; });
        return s;
    }, [knowledge]);
    const correctCount = knowledge.filter((a, i) => a === KNOWLEDGE_QUESTIONS[i].correct).length;
    const determineRisk = useCallback(() => {
        if (risk.includes('capital'))
            return "Low";
        if (risk.includes('recovery') || risk.includes('Hold'))
            return "Medium";
        return "High";
    }, [risk]);
    ...
    const canProceed = useCallback(() => {
        if (step === 1)
            return personal.age && personal.occupation && rawNum(personal.income) && rawNum(personal.expenses);
        if (step === 2)
            return habits.track && habits.emergency && habits.save && habits.budget;
        if (step === 3)
            return !knowledge.includes(-1);
        if (step === 4)
    ...
    useEffect(() => {
        const handler = (e) => {
            if (e.key === 'Enter' && canProceed() && !submitting && !summary)
                handleNext();
        };
    ...
    const pageTrans = { duration: 0.35, ease: [0.22, 0.68, 0.36, 1] };
    if (submitting) {
        return (<div className="min-h-screen bg-white flex flex-col items-center justify-center px-6">
        <div className="fixed inset-0 pointer-events-none" aria-hidden="true">
          <div className="absolute top-1/4 left-1/2 -translate-x-1/2 w-[600px] h-[600px] rounded-full opacity-[0.06]" style={{ background: 'radial-gradient(circle, #2563eb 0%, transparent 70%)' }}/>
        </div>
        <motion.div initial={{ opacity: 0, scale: 0.96 }} animate={{ opacity: 1, scale: 1 }} className="relative z-10 max-w-sm w-full text-center">
          <div className="relative w-20 h-20 mx-auto mb-10">
            ...
            <motion.div className="absolute inset-0 rounded-full border-[3px] border-transparent border-t-blue-600" animate={{ rotate: 360 }} transition={{ duration: 1, repeat: Infinity, ease: "linear" }}/>
    ...
    if (summary) {
        return (<div className="min-h-screen bg-gradient-to-b from-white via-slate-50/40 to-white flex flex-col items-center justify-center px-4 sm:px-6 py-12">
        <div className="fixed inset-0 pointer-events-none" aria-hidden="true">
          <div className="absolute top-[20%] left-1/2 -translate-x-1/2 w-[700px] h-[700px] rounded-full opacity-[0.05]" style={{ background: 'radial-gradient(circle, #2563eb 0%, transparent 70%)' }}/>
        </div>

        {/* Top Action Header */}
        <div className="w-full max-w-3xl flex items-center justify-between mb-6">
          <button onClick={() => navigate('/dashboard')} className="flex items-center gap-2 px-4 py-2 rounded-xl bg-white border border-slate-200 text-slate-700 hover:bg-slate-50 text-sm font-bold shadow-xs transition-all">
            <ArrowLeft className="w-4 h-4"/>
            <span>Dashboard</span>
          </button>

          <div className="flex items-center gap-3">
            <button onClick={handleRetake} className="flex items-center gap-2 px-4 py-2 rounded-xl bg-blue-50 border border-blue-200 text-blue-600 hover:bg-blue-100 text-sm font-bold shadow-xs transition-all">
              <RotateCcw className="w-4 h-4"/>
              <span>Retake Assessment</span>
            </button>
            <button onClick={() => setIsReviewing(!isReviewing)} className="flex items-center gap-2 px-4 py-2 rounded-xl bg-slate-900 text-white hover:bg-slate-800 text-sm font-bold shadow-xs transition-all">
        ...
        <motion.div initial={{ opacity: 0, y: 24 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.45 }} className="relative z-10 max-w-3xl w-full bg-white rounded-[2rem] p-6 sm:p-12 shadow-xl shadow-slate-200/40 border border-slate-100">
          {/* Header */}
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 mb-8">
            ...
            ].map(s => (<div key={s.label} className="bg-slate-50 rounded-2xl p-4 border border-slate-100 text-center">
          ...
          {isReviewing && (<motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: 'auto' }} className="mb-8 border-t border-slate-200 pt-8">
              ...
              <div className="space-y-4">
                {KNOWLEDGE_QUESTIONS.map((q, idx) => {
                    ...
                    return (<div key={idx} className={`p-5 rounded-2xl border transition-all text-left ${isCorrect
                      ...
                      <div className="flex items-start justify-between gap-4 mb-3">
                        ...
                        <span className={`inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-xs font-bold shrink-0 ${isCorrect ? 'bg-emerald-100 text-emerald-700' : 'bg-rose-100 text-rose-700'}`}>
          ...
          <div className="text-left bg-blue-50/60 rounded-2xl p-6 border border-blue-100/70 mb-8">
            ...
            <ul className="space-y-2">
              {["Optimize your emergency fund allocation", "Learn tax-saving fundamentals (Section 80C)", "Automate your monthly investments"].map((t, i) => (<li key={i} className="flex items-center gap-2.5 text-[13.5px] text-slate-600 font-medium">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
            <button onClick={handleRetake} className="w-full flex items-center justify-center gap-2 py-3.5 px-4 text-sm font-bold text-blue-600 bg-blue-50 hover:bg-blue-100 border border-blue-200 rounded-xl transition-all">
              <RotateCcw className="w-4 h-4"/> Retake Assessment
            </button>
            <button onClick={() => setIsReviewing(!isReviewing)} className="w-full flex items-center justify-center gap-2 py-3.5 px-4 text-sm font-bold text-slate-700 bg-slate-100 hover:bg-slate-200 border border-slate-200 rounded-xl transition-all">
              {isReviewing ? <EyeOff className="w-4 h-4"/> : <Eye className="w-4 h-4"/>}
              {isReviewing ? 'Finish Review' : 'Review Answers'}
            </button>
            <button onClick={() => navigate('/dashboard')} className="w-full flex items-center justify-center gap-2 py-3.5 px-4 text-sm font-bold text-white bg-blue-600 hover:bg-blue-700 rounded-xl transition-all shadow-md shadow-blue-600/20">
    ...
    /* ════════════════════════════════════════════════════════════════
       ...
       ════════════════════════════════════════════════════════════ */
    return (<div className="min-h-screen bg-[#f9fafb] text-slate-900 flex flex-col">
      {/* Subtle ambient gradient background */}
      <div className="fixed inset-0 pointer-events-none z-0" aria-hidden="true">
        <div className="absolute top-[10%] left-1/2 -translate-x-1/2 w-[800px] h-[800px] rounded-full opacity-[0.04]" style={{ background: 'radial-gradient(circle, #2563eb 0%, transparent 70%)' }}/>
        <div className="absolute -bottom-[10%] -right-[5%] w-[500px] h-[500px] rounded-full opacity-[0.03]" style={{ background: 'radial-gradient(circle, #6366f1 0%, transparent 70%)' }}/>
      </div>
      ...
      <nav className="bg-white/90 backdrop-blur-xl border-b border-slate-100 px-5 sm:px-8 py-3.5 flex items-center justify-between sticky top-0 z-20">
        ...
        <div className="flex items-center gap-4">
          ...
          <div className="w-28 sm:w-36 h-2 bg-slate-100 rounded-full overflow-hidden">
            <motion.div className="h-full bg-blue-600 rounded-full" animate={{ width: `${progressPct}%` }} transition={{ duration: 0.5, ease: [0.22, 0.68, 0.36, 1] }}/>
          </div>
          {/* Mobile step badge */}
      ...
      <main className="flex-1 flex flex-col relative z-10">
          ...
          <div className="flex flex-col lg:flex-row gap-8 lg:gap-12">
            ...
            <div className="lg:w-[320px] xl:w-[360px] shrink-0">
              <div className="lg:sticky lg:top-24">
                <AnimatePresence mode="wait">
                  <motion.div key={step} initial={{ opacity: 0, x: -12 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -8 }} transition={{ duration: 0.3 }}>
            ...
            <div className="flex-1 min-w-0">
              <AnimatePresence mode="wait">
                <motion.div key={step} variants={pageVariants} initial="initial" animate="animate" exit="exit" transition={pageTrans}>
                  {/* ─── STEP 1: Personal Info ─── */}
                  {step === 1 && (<div className="bg-white rounded-2xl shadow-lg shadow-slate-200/30 border border-slate-100 p-7 sm:p-10">
                  ...
                  {step === 3 && (<AnimatePresence mode="wait">
                      <motion.div key={quizIdx} initial={{ opacity: 0, y: 14 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -14 }} transition={{ duration: 0.28 }} className="bg-white rounded-2xl shadow-lg shadow-slate-200/30 border border-slate-100 p-7 sm:p-10">
                ...
                return (<motion.button key={oIdx} whileHover={{ scale: 1.01, y: -1 }} whileTap={{ scale: 0.99 }} onClick={() => {
                        const nk = [...knowledge];
                        nk[quizIdx] = oIdx;
                        setKnowledge(nk);
                        if (quizIdx < KNOWLEDGE_QUESTIONS.length - 1)
                            setTimeout(() => setQuizIdx(qi => qi + 1), 380);
                    }} aria-pressed={sel} className={`w-full p-4 sm:p-5 rounded-2xl text-left text-[15px] font-medium transition-all duration-200 border-2 ${sel
                        ...
                        : 'bg-slate-50/80 text-slate-700 border-slate-100 hover:border-blue-300 hover:bg-blue-50/40'}`}>
                                <span className="flex items-center gap-3">
                                  <span className={`w-7 h-7 rounded-lg flex items-center justify-center text-[12px] font-black shrink-0 ${sel ? 'bg-white/20 text-white' : 'bg-white text-slate-500 border border-slate-200'}`}>
                        ...
                        <div className="flex justify-between mt-8">
                          <button onClick={() => setQuizIdx(i => Math.max(0, i - 1))} disabled={quizIdx === 0} className="text-sm font-bold text-slate-400 hover:text-slate-700 disabled:opacity-0 transition-all px-3 py-2 rounded-lg">
                            ← Previous
                          </button>
                          <button onClick={() => setQuizIdx(i => Math.min(KNOWLEDGE_QUESTIONS.length - 1, i + 1))} disabled={quizIdx === KNOWLEDGE_QUESTIONS.length - 1 || knowledge[quizIdx] === -1} className="text-sm font-bold text-blue-600 hover:text-blue-700 disabled:opacity-30 transition-all px-3 py-2 rounded-lg">
                            Next →
                          </button>
                        </div>
                      </motion.div>
                ...
                return (<motion.button key={g.label} initial={{ opacity: 0, y: 18 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: idx * 0.04 }} whileHover={{ y: -3 }} whileTap={{ scale: 0.97 }} onClick={() => sel ? setGoals(goals.filter(x => x !== g.label)) : setGoals([...goals, g.label])} aria-pressed={sel} className={`relative p-5 sm:p-6 rounded-2xl flex flex-col items-center text-center gap-3 transition-all duration-200 border-2 ${sel
                        ? 'bg-blue-600 text-white shadow-lg shadow-blue-600/15 border-blue-600'
                        : 'bg-white text-slate-600 border-slate-100 hover:border-blue-300 hover:shadow-md shadow-sm'}`}>
                              {sel && (<motion.div initial={{ scale: 0 }} animate={{ scale: 1 }} className="absolute top-2.5 right-2.5 w-5 h-5 bg-white rounded-full flex items-center justify-center shadow">
                      ...
                      {goals.length > 0 && (<motion.p initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="text-center text-[13px] text-slate-400 font-medium mt-5">
                          {goals.length} goal{goals.length > 1 ? 's' : ''} selected
                        </motion.p>)}
                    </div>)}

                  {/* ─── STEP 5: Risk Profile ─── */}
                  {step === 5 && (<div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                      {RISK_DATA.map((r, idx) => {
                const Icon = r.icon;
                ...
                return (<motion.button key={r.value} initial={{ opacity: 0, y: 18 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: idx * 0.08 }} whileHover={{ y: -4 }} whileTap={{ scale: 0.98 }} onClick={() => setRisk(r.value)} aria-pressed={sel} className={`relative p-6 sm:p-8 rounded-2xl text-left transition-all duration-200 border-2 ${sel ? `${colors.bg} ${colors.border} shadow-lg` : 'bg-white border-slate-100 hover:border-slate-200 shadow-sm hover:shadow-md'}`}>
                            {sel && (<motion.div initial={{ scale: 0 }} animate={{ scale: 1 }} className={`absolute top-3 right-3 w-6 h-6 rounded-full ${colors.iconBg} flex items-center justify-center`}>
        ...
        <div className="sticky bottom-0 z-20 bg-white/90 backdrop-blur-xl border-t border-slate-100 px-5 sm:px-8 lg:px-12">
          <div className="max-w-6xl mx-auto flex justify-between items-center py-4">
            <motion.button whileHover={{ x: -2 }} onClick={() => { if (step === 3 && quizIdx > 0)
        setQuizIdx(0); setStep(s => s - 1); }} disabled={step === 1} className="flex items-center gap-1.5 px-5 py-3 rounded-xl font-bold text-slate-400 hover:text-slate-700 hover:bg-slate-100 transition-all disabled:opacity-0" aria-label="Go back">
            ...
            <div className="flex items-center gap-4">
              ...
              <motion.button whileHover={{ scale: 1.02, y: -1 }} whileTap={{ scale: 0.98 }} onClick={handleNext} disabled={!canProceed()} className="flex items-center gap-2 px-7 py-3.5 bg-blue-600 text-white text-[15px] font-bold rounded-xl hover:bg-blue-700 shadow-lg shadow-blue-600/20 hover:shadow-xl hover:shadow-blue-600/25 transition-all disabled:opacity-40 disabled:cursor-not-allowed disabled:shadow-none" aria-label={step === 5 ? 'Complete assessment' : 'Continue'}>
                {step === 5
